...
class RadioIslandData:
    ...
    def connect_observation_db(self, **db_config):
        ...
        if 'sqlite_database_file' in db_config:
            ...
            try:
                ...
                if self.xenia_obs_db.connectDB(db_config['xenia_obs_db_type'], db_config['xenia_obs_db_user'],
                                               db_config['xenia_obs_db_password'], db_config['xenia_obs_db_host'],
                                               db_config['xenia_obs_db_name'], False):
                    logger.info(f"Succesfully connect to DB: {db_config['xenia_obs_db_name']} at "
                                        f"{db_config['xenia_obs_db_host']}")
                    ret_val = True
                else:
                    logger.error("Unable to connect to DB: {db_config['xenia_obs_db_name']} at "
                                        f"{db_config['xenia_obs_db_host']}")
    ...
    def load_platform_configurations(self, platform_cfg, observation_db_config, nexrad_db_config):
        ...
        self._platform_mappings.from_json(obs_cfg,
                                            db_type='postgresql',
        ...
        self._nexrad_mappings.from_json(nexrad_cfg,
                                           sqlite_database_file=nexrad_db_config['nexrad_database_file'])

    def initialize(self, site, platform_configuration, tide_station, observation_db_config, nexrad_db_config):
        self._site = site
        self._tide_station = tide_station
        self.load_platform_configurations(platform_configuration, observation_db_config, nexrad_db_config)
        try:
        ...
        if len(self._df_column_names) <= 0:
            ...
            for platform_handle in model_site_platforms.platforms:
                ...
                prev_hours = model_site_platforms.get_platform_parameter(platform_handle, 'previous_hours')
                obs_mappings = model_site_platforms.platform_observation_mapping(platform_handle)
                if prev_hours:
                    for hour in range(24, prev_hours + 24, 24):
                        for obs in obs_mappings:
                            if type != 'radarcoverage':
                                ...
                                if obs.target_obs != 'wind_speed' and obs.target_obs != 'wind_from_direction':
                                    col_name = "{name}_{obs}_{hour}".format(name=name,
                                                                            obs=obs.target_obs,
                                                                            hour=hour)
                                    ...
                                    if obs.target_obs == 'wind_speed':
                                        ...
                                        col_name = "{name}_wind_v_{hour}".format(name=name,
                                                                                  hour=hour)
                                        self._df_column_names.append(col_name)
                                        col_name = "{name}_wind_speed_{hour}".format(name=name,
                                                                                hour=hour)
                                        self._df_column_names.append(col_name)
                                    if obs.target_obs == 'wind_from_direction':
                                        col_name = "{name}_wind_from_direction_{hour}".format(name=name,
                                                                                hour=hour)
                                        self._df_column_names.append(col_name)
                                        col_name = "{name}_wind_u_{hour}".format(name=name,
                                                                                  hour=hour)
                                        self._df_column_names.append(col_name)
                else:
                    col_name = "{name}_{obs}".format(name=name,
                                                     obs=obs.target_obs)
                    self._df_column_names.append(col_name)
            ...
            for platform_handle in model_site_nexrad_platforms.platforms:
                org, name, type = platform_handle.split('.')
                prev_hours = model_site_nexrad_platforms.get_platform_parameter(platform_handle, 'previous_hours')
                obs_mappings = model_site_nexrad_platforms.platform_observation_mapping(platform_handle)
                if prev_hours:
                    for hour in range(24, prev_hours + 24, 24):
                        for obs in obs_mappings:
                            ...
                            if hour == 24:
                                self._df_column_names.append("{name}_nexrad_total_1_day_delay".format(name=name))
                                self._df_column_names.append("{name}_nexrad_total_2_day_delay".format(name=name))
                                self._df_column_names.append("{name}_nexrad_total_3_day_delay".format(name=name))
                                self._df_column_names.append("{name}_nexrad_dry_days_count".format(name=name))
                                self._df_column_names.append("{name}_nexrad_rainfall_intensity".format(name=name))

                            self._df_column_names.append("{name}_nexrad_summary_{hour}".format(name=name,
                                                                                           hour=hour))
    ...
    def query_data(self, start_date, end_date):
        ...
        for platform_handle in model_site_platforms.platforms:
            ...
            prev_hours = model_site_platforms.get_platform_parameter(platform_handle, 'previous_hours')
            platform_observations = model_site_platforms.platform_observation_mapping(platform_handle)

            if prev_hours:
                # For observation platforms, we use the get_platform_data function.
                for hour in range(24, prev_hours + 24, 24):
                    self.get_platform_data(platform_handle, platform_observations, start_date, hour)
            else:
                self.get_platform_data(platform_handle, platform_observations, start_date, prev_hours)
        #Get NEXRAD Data
        model_site_nexrad_platforms = self._nexrad_mappings.get_site(self._site)
        for platform_handle in model_site_nexrad_platforms.platforms:
            # Get the previous_hours setting if we are using that.
    ...
    def get_tide_data(self, start_date):
        ...
        if tide_data is not None:
            ...
            self._data_frame.at[self._row_id, f'{self._tide_station}_tide_range'] = tide_range
            self._data_frame.at[self._row_id, f'{self._tide_station}_tide_hi'] = float(tide_data['HH']['value'])
            self._data_frame.at[self._row_id, f'{self._tide_station}_tide_lo'] = float(tide_data['LL']['value'])
            self._data_frame.at[self._row_id, f'{self._tide_station}_tide_stage'] = tide_data['tide_stage']

            logger.debug(f"Finished retrieving tide data for station: {self._tide_station} date: {start_date} in "
                f"{time.time() - start_time} seconds")

        return

    def get_platform_data(self, platform_handle, platform_observations, start_date, prev_hour):
        org, name, type = platform_handle.split('.')
        end_date = start_date
        ...
        for observation in platform_observations:
            try:
                obs_recs = self.xenia_obs_db.session.query(sl_multi_obs) \
                ...
                if observation.target_obs != 'wind_speed' and observation.target_obs != 'wind_from_direction':
                    col_name = f"{name}_{observation.target_obs}_{prev_hour}"
                    if len(obs_recs):
                        avg_data = sum(rec.m_value for rec in obs_recs) / len(obs_recs)
                        logger.debug(f"Platform: {platform_handle} Avg {observation.target_obs}: {avg_data} "
                    ...
                    if observation.target_obs == 'wind_speed' or observation.target_obs == 'wind_from_direction':
    ...
    def get_nexrad_data(self, platform_handle, start_date, previous_hours):
        logger.debug(f"{platform_handle} start retrieving nexrad data datetime: {start_date.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for prev_hour in range(24, previous_hours + 24, 24):
            ...
            radar_val = self._nexrad_db.getLastNHoursSummaryFromRadarPrecip(platform_handle,
                                                                            start_date,
                                                                            prev_hour,
            ...
            if radar_val != None:
                ...
                current_value = None
                logger.error(f"No data available for boundary: {col_name} Date: {start_date}.")
        ...
        prev_dry_days = self._nexrad_db.getPrecedingRadarDryDaysCount(platform_handle,
                                                                      start_date,
                                                                      'precipitation_radar_weighted_average',
                                                                      'mm')
        ...
        rainfall_intensity = self._nexrad_db.calcRadarRainfallIntensity(platform_handle,
                                                                        start_date,
                                                                        60,
                                                                        'precipitation_radar_weighted_average',
                                                                        'mm')
        ...
        logger.debug(
            "Finished retrieving nexrad data datetime: %s" % (start_date.strftime('%Y-%m-%d %H:%M:%S')))
...
class RadioIslandHistoricalData(RadioIslandData):
    ...
    def get_platform_data(self, platform_handle, start_date, prev_hour):
        org, name, type = platform_handle.split('.')
        platform_observations = self._platforms_config.platform_observation_mapping(platform_handle)
        end_date = start_date
        # We have to pair up the wind speed and direction, so we store them in this dict until we finish
        # processing other obs.
        wind_speed_direction = {}
        ...
        for observation in platform_observations:
            try:
                obs_recs = self._db_obj.session.query(sl_multi_obs) \
                    .filter(sl_multi_obs.m_date >= begin_date.strftime('%Y-%m-%dT%H:%M:%S')) \
                    .filter(sl_multi_obs.m_date < end_date.strftime('%Y-%m-%dT%H:%M:%S')) \
                    .filter(sl_multi_obs.sensor_id == observation.sensor_id) \
                    .filter(or_(sl_multi_obs.qc_level == qaqcTestFlags.DATA_QUAL_GOOD, sl_multi_obs.qc_level == None)) \
                ...
                if observation.target_obs != 'wind_speed' and observation.target_obs != 'wind_from_direction':
                    ...
                    if len(obs_recs):
                        avg_data = sum(rec.m_value for rec in obs_recs) / len(obs_recs)
                        logger.debug("Platform: {platform} Avg {obs}: {value} Records used: {rec_count}".format(
    ...
    def get_tide_data(self, start_date):
        logger.debug(f"{self._tide_station} start retrieving tide data date: {start_date}")
        ...
        if self._tide_data_obj is not None:
            ...
            if date_key in self._tide_data_obj:
                ...
                self._data_frame.at[self._row_id, f'{self._tide_station}_tide_range'] = tide_rec['range']
                self._data_frame.at[self._row_id, f'{self._tide_station}_tide_hi'] = tide_rec['hh']
                self._data_frame.at[self._row_id, f'{self._tide_station}_tide_lo'] = tide_rec['ll']
                self._data_frame.at[self._row_id, f'{self._tide_station}_tide_stage']= tide_rec['tide_stage']
            else:
...
def main():
    ...
    parser.add_option("--EnteroDataDirectory", dest="entero_data_dir",
                      help="")
    parser.add_option("--OutputDirectory", dest="output_directory",
                      help="")
    ...
    for tide_station in tide_stations:
        tide_data_files.append(
            os.path.join(config_file.get('tide_stations', 'data_directory'), '{}.csv'.format(tide_station)))

    platform_config = config_file.get("platform_configuration", "setup_file")

    sites_platform_cfg = ModelSitesPlatforms()
    sites_platform_cfg.from_json_file(platform_config, sqlite_database_file=historic_db_file)

    eastern_tz = timezone('US/Eastern')
    utc_tz = timezone('UTC')

    output_file = os.path.join(options.output_directory, "{}_historical.csv".format(wq_site))
    wq_data = RadioIslandHistoricalData(tide_file=tide_data_files[0])
    if wq_data.initialize(wq_site,
                          sites_platform_cfg.get_site(wq_site),
                          '8656483',
                          {'sqlite_database_file': historic_db_file},
                          {'nexrad_database_file': historic_db_file}):

        header = ["Station", "DateTime", "Entero SSM", "Entero GM", "Entero SSM_CFU", "Entero GM2"]
