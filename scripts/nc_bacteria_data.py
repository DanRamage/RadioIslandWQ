    ...
    try:
        ...
        with open(data_filename, "wb") as output_file:
            ...
            if req.status_code == 200:
                ...
                logger.info(
                    "Saved sample data in %f seconds" % (time.time() - start_time)
                )
            else:
                logger.info(
...
def parse_excel_data(file, monitoring_sites, wq_data_collection):
    ...
    results_ndx = (
        ...
        county_ndx
    ) = site_id_ndx = area_ndx = entero_gm_ndx = entero_ssm_ndx = date_ndx = None
    ...
    for row_ndx, data_row in enumerate(sheet.get_rows()):
        if row_ndx != 0:
            ...
            site_id = "{id}{site}".format(
                id=data_row[area_ndx].value.strip(),
                site=data_row[site_id_ndx].value.strip(),
            )
            try:
                matched = False
                if (
                        current_excel_station is None
                        or current_excel_station != station_name
                ):
                    for ndx, site_nfo in enumerate(monitoring_sites):
                        if site_id == site_nfo.name:
                            logger.info(
                                "Processing row: {} Site: {} Name: {}".format(
                                    row_ndx, site_id, station_name
                                )
                            )
                            matched = True
                ...
                if matched:
                    try:
                        ...
                        try:
                            date_val = xlrd.xldate.xldate_as_datetime(
                                data_row[date_ndx].value, wb.datemode
                            )
                        except Exception as e:
                            logger.exception(e)
                            break
                        wq_sample_rec.date_time = (
                            est_tz.localize(date_val)
                        ).astimezone(utc_tz)
                        ...
                        wq_sample_rec.entero_ssm_cfu = data_row[
                            entero_ssm_cfu_ndx
                        ].value
                        ...
                        logger.info(
                            "Site: %s Date: %s SSM: %s GM: %s"
                            % (
                                wq_sample_rec.station,
                                wq_sample_rec.date_time,
