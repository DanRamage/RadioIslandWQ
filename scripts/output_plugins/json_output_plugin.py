    ...
    def run(self, **kwargs):
        ...
        try:
            ...
            with open(self.json_outfile, "w") as json_output_file:
                station_data = {"features": [], "type": "FeatureCollection"}
                features = []
                        ...
                        features.append(
                            {
                                ...
                                "geometry": {
                                    "type": "Point",
                                    "coordinates": [
                                        site_metadata.object_geometry.x,
                                        site_metadata.object_geometry.y,
                                    ],
                                },
                                "properties": {
                ...
                try:
                    ...
                    logger.exception(e)
            logger.debug(
                "Finished json output in %f seconds." % (time.time() - start_time)
            )
        except IOError as e:
            logger.exception(e)
        return
