    ...
    def initialize_plugin(self, **kwargs):
        try:
            ...
            try:
                ...
                self.mailhost = config_file.get(
                    "email_report_output_plugin", "mailhost"
                )
                self.mailport = config_file.getint("email_report_output_plugin", "port")
                self.fromaddr = config_file.get(
                    "email_report_output_plugin", "fromaddr"
                )
                self.toaddrs = config_file.get(
                    "email_report_output_plugin", "toaddrs"
                ).split(",")
                self.subject = config_file.get("email_report_output_plugin", "subject")
                self.user = config_file.get("email_report_output_plugin", "user")
                self.password = config_file.get(
                    "email_report_output_plugin", "password"
                )
    ...
    def run(self, **kwargs):
        try:
            ...
            with open(out_filename, "w") as report_out_file:
                ...
                results_report = mytemplate.render(
                    nowcast_site=self.nowcast_site_name,
            ...
            try:
                ...
                logger.exception(e)
            logger.debug(
                f"email_output_plugin finished in {time.time() - start_time} seconds."
            )
