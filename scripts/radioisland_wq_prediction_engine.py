...
class radioisland_prediction_engine(wq_prediction_engine):
    ...
    def run_wq_models(self, **kwargs):
        ...
        try:
                ...
                try:
                    ...
                    try:
                        if len(model_list):
                            ...
                            obs_platform_config = config_file.get(site.name, "platform_configuration")
                            ...
                            wq_data.initialize(site.name,
                                               platforms_config_json,
                                               tide_station,
                                               observation_db_config,
                                               {'nexrad_database_file': xenia_nexrad_db_file}
                                               )
                            wq_data.query_data(kwargs['begin_date'], kwargs['begin_date'])
                            pd_df = wq_data.get_data_frame()
                            model_list.runTestsDF(pd_df)
                            total_test_time = sum(model.test_time for model in model_list.models)
                            self.logger.debug(
                                "Site: %s total time to execute models: %f ms" % (site.name, total_test_time * 1000))
            ...
            self.logger.debug(f"Total time to execute all sites models: {total_time * 1000} ms")
    ...
    def build_test_objects(self, site_name, config_file):
        ...
        try:
            ...
            if machine_learning_model_count:
                machine_learning_model_count = model_config_file.getint("settings", "machine_learning_model_count")
                self.logger.debug(f"Site: {site_name} ML Model count: {machine_learning_model_count}")
            model_list = []
            for cnt in range(machine_learning_model_count):
                try:
                    #Get the name of the model object this model uses.
                    model_object_name = model_config_file.get(f"machine_learning_model_{(cnt + 1)}", "model_object")
                    if model_object_name == 'cbm_model_classifier':
                        #Get the category of the model this represents.
                        type = model_config_file.get(f"machine_learning_model_{(cnt + 1)}", "type")
                        #This is the name we use on the reports to identify the model.
                        model_name = model_config_file.get(f"machine_learning_model_{(cnt + 1)}", "name")
                        #This is the filename of the model we user for the nowcast.
                        model_filename = model_config_file.get(f"machine_learning_model_{(cnt + 1)}", "model_file")
                        false_positive_threshold = model_config_file.get(f"machine_learning_model_{(cnt + 1)}",
                                                                              "false_positive_threshold",
                                                                              fallback=None)
                        false_negative_threshold = model_config_file.get(f"machine_learning_model_{(cnt + 1)}",
                        ...
                        self.logger.debug(f"Site: {site_name} Model name: {model_name} model file: {model_filename}")
