...
from wq_output_results import wq_samples_collection
    ...
    try:
        for site in wq_sites:
            enetero_filename = os.path.join(
                options.advisory_files_dir, "{site}.csv".format(site=site.name)
            )
            logger.debug(
                "Creating site: {site_name} advisories file: {file}".format(
                    site_name=site.name, file=enetero_filename
                )
            )
