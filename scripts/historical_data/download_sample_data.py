...
def download_sample_data(destination_dir, start_year, end_year):
    ...
    try:
        data_filename = os.path.join(destination_dir, "{start_year}-{end_year}_sampledata.xls".format(start_year=start_year,
                                                                                                      end_year=end_year))
        with open(data_filename, "wb") as output_file:
            ...
            if req.status_code == 200:
                ...
                print("ERROR: Unable to download data. Status Code: {status}".format(req.status_code))
...
def main():
    ...
    parser.add_option("--DestinationDir", dest="destination_dir",
                      help="Destination directory to save the sampling data.")
    parser.add_option("--StartYear", dest="start_year", type="int",
                      help="Date to begin the data downloads")
    parser.add_option("--EndYear", dest="end_year",type="int",
                      help="Date to end the data downloads")
