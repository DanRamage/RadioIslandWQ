...
from xenia_obs_map import json_obs_map
...
class build_database:
    def __init__(self, historic_db_name, platforms_config, data_saver_log_configfile):
        ...
        if self._historical_db.connectDB(
                "sqlite", None, None, self._historic_db_filename, None, False
        ):
        ...
        self._record_saver = SQLiteMPDataSaver(
            self._historic_db_filename, data_saver_log_configfile, self._data_queue
        )
        ...
        try:
            for platform_cfg in self._platforms_config:
                ...
                if data_provider == "nos":
                    start_date = begin_date - timedelta(
                        hours=platform["previous_hours"]
                    )
    ...
    def setup_platform(self, xenia_platform, observations):
        ...
        if platform_id is None:
            self._historical_db.newPlatform(
                self._row_entry_date, xenia_platform, 0.0, 0.0
            )
        ...
        self._obs_map.build_db_mappings(
            sqlite_database_file=self._historic_db_filename,
            ...
            if len(obs.source_obs):
                try:
                    ...
                    for index in obs_df.index:
                        ...
                        if add_record:
                            m_obs = sl_multi_obs(
                                row_entry_date=self._row_entry_date.strftime(
                                    "%Y-%m-%d %H:%M:%S"
                                ),
                                platform_handle=xenia_platform,
                            ...
                            if obs.source_obs == "wind":
                                wind_dir_obs = self._obs_map.get_rec_from_xenia_name(
                                    "wind_from_direction"
                                )
