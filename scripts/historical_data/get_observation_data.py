...
def main():
    ...
    for site in wq_sites:
        ...
        platform_data = build_database(
            historic_db_file, platform_config, datasaver_config_file
        )
        entero_data_file = os.path.join(
            entero_data_dir, "{site}.csv".format(site=site.name)
        )
        entero_dates = get_entero_dates(entero_data_file)
        platform_data.initialize()
        for entero_date in entero_dates:
            logger.info(
                "Site: {sitename} Date: {date}".format(
                    sitename=site.name, date=entero_date
                )
            )
